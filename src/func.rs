...
pub trait Func<N: Num> {
	...
	fn eval(&self, args: &[Term<N>], ctx: &Context<N>, supp: Option<Arc<RwLock<dyn SupplementaryDataAdapter<N>>>>) -> Calculation<N>;
}

/// Blanket impl for closures
impl<T, N: Num> Func<N> for T
where
	T: Fn(&[Term<N>], &Context<N>, Option<Arc<RwLock<dyn SupplementaryDataAdapter<N>>>>) -> Calculation<N>,
{
	fn eval(&self, args: &[Term<N>], ctx: &Context<N>, supp: Option<Arc<RwLock<dyn SupplementaryDataAdapter<N>>>>) -> Calculation<N> {
		self(args, ctx, supp)
	}
